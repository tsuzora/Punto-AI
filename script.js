class PuntoAI {
    ...
    findBestMove(playerId) {
        ...
        for (let cardIdx of handIndices) {
            ...
            candidates.forEach(pos => {
                const validation = this.game.checkMoveValidity(pos.x, pos.y, card.value, p);
                ...
                if (validation.valid) {
                    ...
                    if (result.type !== 'Normal' && significantMovesFound < 4) {
                         this.logs.push(`> Candidate (${pos.x+this.game.CENTER-5},${pos.y+this.game.CENTER-5}): ${result.type}`);
                         significantMovesFound++;
                    }

                    if (score > maxScore) {
                        maxScore = score;
                        bestMove = { lx: pos.x, ly: pos.y, cardIndex: cardIdx, type: result.type };
                    }
                }
            });
        }
        ...
        if (bestMove) {
            ...
            this.logs.push(`Target: ${bestMove.lx+this.game.CENTER-5}, ${bestMove.ly+this.game.CENTER-5}`);
            this.logs.push(`Card: ${p.hand[bestMove.cardIndex].color} ${p.hand[bestMove.cardIndex].value}`);
        } else {
    ...
    calculateHeuristic(x, y, card, player) {
        ...
        this.game.gridData[key] = { color: card.color, value: card.value, ownerId: player.id };
        ...
        // f3: Setup (3 in a row)
        if (type === 'Normal') { // Only check if not already winning
        ...
        if (type === 'Normal' || type === 'f3: Build Line') {
            ...
            for (let opp of opponents) {
                opp.colors.forEach(oppColor => {
                    ...
                    this.game.gridData[key] = { color: oppColor, value: 9, ownerId: opp.id };
...
class PuntoTactics {
    ...
    updateStatsUI(playerId, latency, nodes) {
        ...
        elLat.style.color = latency < 50 ? 'var(--neon-green)' : (latency < 100 ? 'var(--neon-yellow)' : 'var(--neon-red)');
    ...
    showInstructions() { this.instructionsModal.classList.add('active'); }
    ...
    init(count, mode = 'pvp') { // mode: 'pvp', 'pve', 'cvc'
    ...
    createPlayers(count, mode) {
        this.players = [];
        const configs = [
            { id: 0, name: mode === 'cvc' ? "CPU 1" : "P1", colors: count === 2 ? ['red', 'blue'] : ['red'] },
            { id: 1, name: mode === 'cvc' ? "CPU 2" : (mode === 'pve' ? "CPU" : "P2"), colors: count === 2 ? ['green', 'yellow'] : ['blue'] },
    ...
    initStatsUI() {
        // Hide all first
        document.querySelectorAll('.stat-row').forEach(el => el.classList.add('hidden'));
    ...
    renderHand() {
        ...
        // UI Ref: The label above the hand
        const label = document.getElementById('hand-label');
        
        // Hide hand if CPU turn
        if (p.type === 'cpu') {
            this.handContainer.innerHTML = '<div style="color:#666; font-style:italic;">CPU thinking...</div>';
            if(label) label.style.color = '#666'; // Dim the label
            return;
        }

        // Color the label to match the player
        if(label) {
        ...
        p.hand.forEach((card, index) => {
            ...
            if (this.selectedCardIndex === index) {
                ...
                if(label) label.textContent = `Place ${card.color.toUpperCase()} ${card.value}`;
            }
    ...
    checkMoveValidity(tx, ty, cardValue, player) {
        ...
        if (existing) {
            if (existing.ownerId === player.id) return { valid: false, reason: "self-overwrite" }; // P01: p(x,y) != w
            if (cardValue <= existing.value) return { valid: false, reason: "value-too-low" };
        }
    ...
    highlightValidMoves() {
        ...
        const p = this.players[this.currentPlayerIndex];
        if (p.type === 'cpu' || this.selectedCardIndex === null || !p.hand[this.selectedCardIndex]) return;
    ...
    countLines(x, y, color) {
        ...
        for (let [dx, dy] of dirs) {
            ...
            for(let i=1; i<4; i++) {
                const k = `${x + dx*i},${y + dy*i}`;
                if(this.gridData[k]?.color === color) count++; else break;
            }
            for(let i=1; i<4; i++) {
                const k = `${x - dx*i},${y - dy*i}`;
                if(this.gridData[k]?.color === color) count++; else break;
